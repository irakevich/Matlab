function [V,cWRR, Xwgr,Ewrz,HEwrz,WRRS]=HeuristicMethodSpecialNeighbourhood_V6_P(Xwgr,Ewrz,HEwrz,V,wf,C6wr)
...
krok=0;
%disp('----HeuristicMethodSpecialNeighbourhood V6_P----');
while(theEnd~=1)&&(V~=0)
    cWRR = ones(W,R,R-1)*M;
   ...
   %--V
    ...
    [Vwr]=SpecialNeighbourhood_V6_P(Xwgr,Ewrz,HEwrz,wf,C6wr); % if wf >1 ==> matrix Vwr=[zeros(wf-1,R);Vwr(wf:W,R)]; 
    ...
    if min(min(min(cWRR))) <V
      ...
      %  [Vhm,cWRR, Xwgr,Ewrz,HEwrz]=HeuristicMethodNew(Xwgr,Ewrz,HEwrz,V,0,wf, C6wr);%SN==0,wf,cup=0
       % if Vhm<V 
        %    V=Vhm;
         %   CalculateTheCostOfAllAssignmentVVVVV(Xwgr,Ewrz,HEwrz,C6wr);  %nie jest potrzebne                  
        %else    
