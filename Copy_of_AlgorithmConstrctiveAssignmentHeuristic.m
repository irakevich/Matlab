function [Xwgr, Ewrz,HEwrz,Cwgr,Cwrg]=Copy_of_AlgorithmConstrctiveAssignmentHeuristic(Hwgz,Awgz)
...
%                disp('     --- g ---');disp(g);  
    ...
    for zz=1:Z
        ...
        %if TeamXRefCount(1,zz) > 0
         %   V1Ira(zz)= sum( VCombination(floor(TeamXRefCount(1,zz))).*TeamInGame_z);
        %else
         %   V1Ira(zz) = 0;
        %end
    end;
        ...
        if V1Ira<0
%                   disp('TeamXRefCount');disp(TeamXRefCount);    
%                   disp('combination');disp(combination);
%                   disp('TeamInGame_z');disp(TeamInGame_z);
    ...
    % --V3    
        TMPw1z = HEwrz(:,r,:);
        TMPwz = TMPw1z(:,:); %splaszczyc wynik do macierzy
        V3 = sum(max(sum(TMPwz, 1).*HomeInGame_z+1 - Oh, 0));
        %disp('V3');disp(V3);
        %Crg(r,g) = V3;
     % --V4
