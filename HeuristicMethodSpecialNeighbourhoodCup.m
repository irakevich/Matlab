function [V,cWRR, Xwgr,Ewrz,HEwrz]=HeuristicMethodSpecialNeighbourhoodCup(Xwgr,Ewrz,HEwrz,V,cup)
...
while(theEnd~=1)&&(V~=0)
    ...
    if min(min(min(cWRR))) <V
        ...
        [Vhm,cWRR, Xwgr,Ewrz,HEwrz]=HeuristicMethodCup(Xwgr,Ewrz,HEwrz,V,0,wf,0);%SN==0,wf,cup=0
        if Vhm<V 
            V=Vhm;
            CalculateTheCostOfAllAssignmentVVVVV(Xwgr,Ewrz,HEwrz);  %nie jest potrzebne                  
        else    
